        ...
        try:
            ...
            for zone in zone_cols:
                for flow_type in flow_types:
                    old_col = (
                        f"{zone}.{flow_type}" if flow_type != "Flow" else f"{zone}"
                    )
                    new_col = (
                        f"{zone} {flow_type}"
                        if zone != "Total"
                        else f"Total {flow_type}"
                    )
            ...
            flow_columns = [
                ...
                if any(x in col for x in ["Imp", "Exp", "Flow"])
            ]
            ...
            logger.error(f"Error parsing intertie schedule flow file {url}: {e}")
            return pd.DataFrame()
