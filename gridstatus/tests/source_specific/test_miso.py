...
class TestMISO(BaseTestISO):
    ...
    """get_generation_fuel_mix_by_area"""

    generation_fuel_mix_by_area_columns = [
        "Interval Start",
        "Interval End",
        "Publish Time",
        "Area",
    ...
    generation_fuel_mix_by_area_fuels = [
        "Coal",
        "Gas",
        "Nuclear",
        "Hydro",
    ...
    def _check_generation_fuel_mix_by_area(self, df: pd.DataFrame):
        assert df.columns.tolist() == self.generation_fuel_mix_by_area_columns
        ...
        assert df["Area"].dtype == "string"

        for col in self.generation_fuel_mix_by_area_fuels + ["Total"]:
            assert df.dtypes[col] == "Float64"

        # All four areas present, in order, for every interval
        assert set(df["Area"].unique()) == {"Central", "North", "South", "MISO"}

        # Intervals are hourly and one row per (interval, area)
        assert not df.duplicated(["Interval Start", "Area"]).any()
        assert (
            df["Interval End"] - df["Interval Start"] == pd.Timedelta(hours=1)
        ).all()

        # Each area reports 24 hours for each market date
        per_day_area = df.groupby([df["Interval Start"].dt.date, "Area"]).size()
        assert (per_day_area == 24).all()

        # South does not report Storage, so it is always null for that area
        assert df[df["Area"] == "South"]["Storage"].isna().all()

        # MISO system total equals the sum of the three physical areas per fuel.
        # Blank values are treated as zero, matching how the source totals them.
        fuel_cols = self.generation_fuel_mix_by_area_fuels + ["Total"]
        miso_total = (
            df[df["Area"] == "MISO"]
            .set_index("Interval Start")[fuel_cols]
            .sort_index()
            .astype(float)
            .fillna(0)
        )
        area_sum = (
            df[df["Area"] != "MISO"]
            .groupby("Interval Start")[fuel_cols]
            .sum()
            .sort_index()
            .astype(float)
            .fillna(0)
        )
        diff = (miso_total - area_sum).abs()
        assert (diff.max() < 1.0).all()

    def test_get_generation_fuel_mix_by_area_real_time(self):
        start, end = "2024-02-15", "2024-02-17"
        cassette_name = "test_get_generation_fuel_mix_by_area_real_time.yaml"
    ...
    def test_get_generation_fuel_mix_by_area_day_ahead(self):
        start, end = "2024-02-15", "2024-02-17"
        cassette_name = "test_get_generation_fuel_mix_by_area_day_ahead.yaml"

        with miso_vcr.use_cassette(cassette_name):
            df = self.iso.get_generation_fuel_mix_by_area_day_ahead(start, end=end)
    ...
    def test_get_generation_fuel_mix_by_area_no_data(self):
        ...
        future = (
            pd.Timestamp.now(tz="EST").normalize() + pd.Timedelta(days=10)
        ).strftime(
            "%Y-%m-%d",
        )
        cassette_name = "test_get_generation_fuel_mix_by_area_no_data.yaml"

        with miso_vcr.use_cassette(cassette_name):
            with pytest.raises(NoDataFoundException):
                self.iso.get_generation_fuel_mix_by_area_real_time(future)
