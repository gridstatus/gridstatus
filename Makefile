...
PYTEST_CMD := python -m pytest -s -vv gridstatus/ -n auto --reruns 5 --reruns-delay 3
...
.PHONY: test-cov
test-cov:
	$(PYTEST_CMD) -m "not slow" --cov=gridstatus --cov-config=./pyproject.toml --cov-report=xml:./coverage.xml
